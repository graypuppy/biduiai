...
import DeductionConfirmModal from './components/modals/DeductionConfirmModal';
...
export default function App() {
  ...
  const applyTemplate = (tplId: string) => {
    ...
    if (tpl) {
      ...
      const types = biddingDocFile 
        ? tpl.config.types 
        : tpl.config.types.filter((t: string) => t !== '经济标比对');
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < fileArray.length; i++) {
        ...
        validFiles.push({
          ...
          name: file.name,
          size: file.size,
          pages: Math.max(12, Math.floor(file.size / 25600) + Math.floor(Math.random() * 20)),
          status: '未比对',
  ...
  const getRequiredPages = () => {
    return files.filter(f => f.status === '未比对' || f.status === '比对中').reduce((acc, f) => acc + (f.pages || 50), 0);
  };
  ...
  const confirmDeductionAndStart = () => {
    const requiredPages = getRequiredPages();
    
    // Deduct pages
    setUserBalance(prev => Math.max(0, prev - requiredPages));
    ...
    const newNotification: Notification = {
      ...
      type: 'task',
      title: '比对额度已扣减',
      content: `本次项目比对成功扣减 ${requiredPages} 页。您的账户剩余额度为 ${userBalance - requiredPages} 页。`,
      time: '刚刚',
      read: false
    };
  ...
  const handlePaymentSuccess = (addedPages?: number) => {
    setShowPurchaseModal(false);
    
    if (addedPages) {
      setUserBalance(prev => prev + addedPages);
      // Add a system notification
      const newNotification: Notification = {
        id: Date.now(),
        type: 'system',
        title: '充值成功',
        content: `您已成功充值 ${addedPages} 页比对额度。当前账户余额为 ${userBalance + addedPages} 页。`,
        time: '刚刚',
        read: false
      };
      setNotifications(prev => [newNotification, ...prev]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <DeductionConfirmModal 
        ...
        onConfirm={confirmDeductionAndStart}
        files={files}
        userBalance={userBalance}
        requiredPages={getRequiredPages()}
        onRechargeClick={() => {
          setShowDeductionModal(false);
          setPurchaseInitialView('main');
