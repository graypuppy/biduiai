...
export function useAppState() {
  ...
  const [selectedCheckTypes, setSelectedCheckTypes] = useState<string[]>(
    ALL_CHECK_TYPES.filter(t => t !== '经济标比对')
  );
  ...
  // Auto check/uncheck "经济标比对" based on whether biddingDocFile is present
  useEffect(() => {
    if (biddingDocFile) {
      setSelectedCheckTypes(prev => prev.includes('经济标比对') ? prev : [...prev, '经济标比对']);
    } else {
      setSelectedCheckTypes(prev => prev.filter(t => t !== '经济标比对'));
    }
  }, [biddingDocFile]);
  ...
  const [selectedSku, setSelectedSku] = useState<string>('pages500');
